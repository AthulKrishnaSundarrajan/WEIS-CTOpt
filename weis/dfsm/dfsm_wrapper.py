...
def  generate_wind_files_local(fst_vt, modopt, inputs, discrete_inputs, FAST_runDirectory, FAST_namingOut, wind_directory):
    ...
    if modopt['OpenFAST']['from_openfast']:
        reg_traj = modopt['OpenFAST']['regulation_trajectory']
    ...
    for i_case, case_inputs in enumerate(dlc_generator.openfast_case_inputs):
        ...
        case_list_i, case_name_i = CaseGen_General(case_inputs, FAST_runDirectory, FAST_namingOut, filename_ext=f'_DLC{dlc_label}_{i_case}')
    ...
    for case_i, wt, wf in zip(case_list,WindFile_type,WindFile_name):
        ...
        case_i[('InflowWind','FileName_Uni')] =FAST_runDirectory + os.sep + wf
        case_i[('InflowWind','FileName_BTS')] =FAST_runDirectory + os.sep+ wf
...
def dfsm_wrapper(fst_vt, modopt, inputs, discrete_inputs, FAST_runDirectory = None, FAST_namingOut = None,mpi_options = None):
    ...
    if usecase == 'closed-loop-simulation' :
        ...
        for icase,sim_result in enumerate(sim_outputs):
            ...
            # get output
            ae_output = AeroelasticOutput(OutData, dlc = 'dfsm_'+str(i_case),  magnitude_channels = magnitude_channels,fatigue_channels = fatigue_channels )
    ...
    return cruncher,case_list, case_name, ct, dlc_generator, TMax, TStart
